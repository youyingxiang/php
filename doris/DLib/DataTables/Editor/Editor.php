	...
	private function _ssp_filter ( $query, $http )
	{
			...
			$query->where( function ($q) use (&$that, &$fields, $http) {
				for ( $i=0 ; $i<count($http['columns']) ; $i++ ) {
					if ( $http['columns'][$i]['searchable'] == 'true' ) {
						...
						//$q->or_where( $field, '%'.$http['search']['value'].'%', 'like' );//2014/08/01 高勇改
						$q->or_where( $field, '%'.$http['search']['value'].'%', 'like binary' );
					}
				}
			} );
		}
		...
		for ( $i=0, $ien=count($http['columns']) ; $i<$ien ; $i++ ) {
			...
			if ( $search && $column['searchable'] == 'true' ) {
				//$query->where( $this->_ssp_field( $http, $i ), '%'.$search.'%', 'like' );//2014/08/01 高勇改 9730827@qq.com
			
				$query->where( $this->_ssp_field( $http, $i ), '%'.$search.'%', 'like binary' );
			}
		}
	}
