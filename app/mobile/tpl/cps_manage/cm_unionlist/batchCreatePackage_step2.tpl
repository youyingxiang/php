...
<div class="panel panel-default" >
        ...
        <div class="form-group android_package" <?php if(!$soucePackageList[0]['is_android']){echo 'style="display:none"';} ?>  >
            ...
            <div class="col-sm-10">
                <p class="form-control-static"><input type="text" name="package_name"  class="form-control"  id="package_name"></p>
            </div>
        </div>
...
<script>
    ...
    $(function(){
        ...
        function ajax_create(cur_index){
                ...
                var postdata = 'package_file_id='+package_file_id+'&union_id='+channel_id+'&package_id='+package_id;
                if(is_android == 1){
                ...
                $.ajax({
                    type:'post',
                    data:postdata,
                    url:'/index.php?m=cps_manage&c=cm_unionlist&a=batchCreatePackage_ajax',
                    //async: false,
                    success:function(msg){
                        var res = eval("("+msg+")");
                        if(res.state == 0){
