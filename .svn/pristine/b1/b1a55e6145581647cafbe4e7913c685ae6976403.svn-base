...
//			$this->error('请先登录','/index.php?m=index&c=login');
//		}
  ...
  //             //exit("asd");
	...
	public function ensureAdmin($back_url=null){
		if( empty($_SESSION['admin'])){
			...
			//dd($_SESSION['admin']['id']);
			$sql_find = "select user_coins from `tb_sys_user` where id= ".$_SESSION['admin']['id']."";
			$res = Doris\DDB::fetch($sql_find)['user_coins'];
			$_SESSION['admin']['user_coins'] = $res;
