...
window.onload=function(){
    datalist_ajax_url = js_para.ajax_url;
...
function caculate(){
    ...
    var url = getNewListAjaxUrl( controller_url + "&a=showorder_calc" );
    //刷新计算结果
    $.ajax({async: true, type : "get",url:url, data:{},
        ...
        success : function(result) {
            if(result.code==0){
                ...
                $("#id-status").html(result.button);
                $("#id-date").text(result.date);
                $("#id-settle").text(result.data.deduct);
            }else{
                alert("错误："+result.msg);
            }
